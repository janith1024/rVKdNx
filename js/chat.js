...
//        var chatmessage = '<p>' + $('.chat-input').val() + '</p>';
    var chatmessage = $('.chat-input').val() + '<br>';

    if (window.getUserId() == 1) {
        $("#chatMainList").append("<li><img src='http://34.192.103.7:8080/t-talk/images/photo-1.jpg'>" +
            "<div class='message'>" + chatmessage + "</div></li>");
    } else if (window.getUserId() == 2) {
        $("#chatMainList").append("<li><img src='http://34.192.103.7:8080/t-talk/images/photo-2.jpg'>" +
            "<div class='message'>" + chatmessage + "</div></li>");
    } else if (window.getUserId() == 3) {
        $("#chatMainList").append("<li><img src='http://34.192.103.7:8080/t-talk/images/photo-3.jpg'>" +
            "<div class='message'>" + chatmessage + "</div></li>");
    }
    ...
    //alert("sender" + window.getUserId());
    //alert("receiver" + window.currentUserId);
    ...
    $.ajax({
        ...
        data: JSON.stringify({
            message: chatmessage,
            sender: window.getUserId(),
            receiver: window.currentUserId,
...
$('#add-contact-floater').on('click', function () {
    if ($(this).hasClass('active')) {
        ...
        setModal('add');
        $('#contact-modal').one('click', '.btn.save', function () {
            $('.list-account > .list').prepend('<li><img src="images/user.jpg"><span class="name">' + $('#new-user').val() + '</span><i class="mdi mdi-menu-down"></i></li>');
            closeModal();
        });
    }
});
